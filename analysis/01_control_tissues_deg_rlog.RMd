---
title: "Control samples, tissue DEG and rlog"
author: "Eli Roberson"
date: "May 02, 2024"
...
deseq_design_matrix <- info[ , c( "run_batch", "tissue" ), drop = FALSE ]
```
...
results <- results( 
  object = deseqObj, contrast = c( 'tissue', 'plasma_exosomes', 'urine_exosomes' ) )  %>%
	as.data.frame( . ) %>%
...
tmp <- assay( rlog( object = deseqObj, 
                    blind=FALSE ) ) %>%
  	as.data.frame( . ) %>%
  	rownames_to_column( "gene_id" ) %>%
  	write_csv( path =  here( 'results', "control_only_tissue_rlog.csv.gz" ) ) 
```
